...
train_loader = DataLoader(train, batch_size=args.batch_size, shuffle=True, num_workers=8)
valid_loader = DataLoader(valid, batch_size=args.batch_size, shuffle=False, num_workers=8)
test_loader = DataLoader(test, batch_size=args.batch_size, shuffle=False, num_workers=8)
model = None
...
if args.resume and model is not None:
...
def train(epoch):
    ...
    acc_all = 0.0
    counter = 0
    for batch_idx, (image, target, meta_target, meta_structure, embedding, indicator) in enumerate(train_loader):
        counter += 1
        if args.cuda:
            image = image.cuda()
            target = target.cuda()
        ...
        acc_all += acc
    if counter > 0:
        print("Avg Training Loss: {:.6f}".format(loss_all / counter))
...
def validate(epoch):
    ...
    acc_all = 0.0
    counter = 0
    for batch_idx, (image, target, meta_target, meta_structure, embedding, indicator) in enumerate(valid_loader):
        counter += 1
        if args.cuda:
            image = image.cuda()
            target = target.cuda()
...
def test(epoch):
    ...
    acc_all = 0.0
    counter = 0
    for batch_idx, (image, target, meta_target, meta_structure, embedding, indicator) in enumerate(test_loader):
        counter += 1
        if args.cuda:
            image = image.cuda()
            target = target.cuda()
